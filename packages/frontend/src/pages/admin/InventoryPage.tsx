...
import { DataTable, Table, TableHead, TableRow, TableHeader, TableBody, TableCell, Button, NumberInput, Toggle, Tag } from '@carbon/react'
...
export default function InventoryPage() {
  ...
  const { data, isLoading } = useInventory({ lowStock, page, pageSize: 20 })
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h1>Inventory</h1>
        <Toggle id="low-stock-toggle" labelText="부족 재고만" toggled={lowStock} onToggle={(v: boolean) => { setLowStock(v); setPage(1) }} size="sm" />
      </div>
      <DataTable rows={rows} headers={headers} data-testid="inventory-table">
                      ...
                      <TableCell key={cell.id}>
                        {cell.info.header === 'availableStock' ? (
                          ...
                          editId === row.id ? (
                            ...
                            <Button size="sm" kind="ghost" data-testid={`edit-stock-${row.id}`} onClick={() => { setEditId(row.id); setEditVal(item?.stock ?? 0) }}>Edit</Button>
                          )
                        ) : cell.value}
                      </TableCell>
                    ))}
