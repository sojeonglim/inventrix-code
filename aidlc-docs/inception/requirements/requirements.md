...
- 3개 역할 도입:
  - **관리자 (Admin)**: 전체 시스템 관리, 역할 할당, 상품/주문/재고 관리, 분석 대시보드 조회, 보안 모니터링
  - **직원 (Staff)**: 주문/상품/재고 조회, 재고 수정, 배송 상태 변경 (processing→shipped→delivered)
  - **고객 (Customer)**: 상품 탐색, 주문 생성, 본인 주문 조회/취소
